...
keys, list) that can be transmitted to practically any language or platform. It can also
work with a wide variety of formats, though with some limitations:
...
* JSON can be read into ARD. However, because JSON has fewer types (no integers, only floats),
  ARD can be translated to JSON but some type information would be lost unless it were to be encoded
...
The data is untreated and not validated.
