...
func (self *CloutContext) NewCoercible(data ard.Value, meta ard.StringMap, functionCallContext FunctionCallContext) (Coercible, error) {
	if notation, ok := data.(ard.StringMap); ok {
		if data, ok := notation["$primitive"]; ok {
			return self.NewValue(data, notation, meta, functionCallContext)
		} else if data, ok := notation["$list"]; ok {
			if list, ok := asList(data); ok {
				return self.NewListValue(list, notation, meta, functionCallContext)
			} else {
				return nil, fmt.Errorf("malformed \"$list\", not a list: %T", data)
			}
		} else if data, ok := notation["$map"]; ok {
			if list, ok := asList(data); ok {
				return self.NewMapValue(list, notation, meta, functionCallContext)
			} else {
			...
			if map_, ok := asStringMap(data); ok {
				return self.NewFunctionCall(map_, notation, meta, functionCallContext)
			} else {
